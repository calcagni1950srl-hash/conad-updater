...
def sleep_jitter(base, spread=1.5):
    time.sleep(base + random.uniform(0, spread))

def request_json(session, url, params=None):
    # Il 481 osservato nel test precedente viene trattato come blocco temporaneo.
    waits_481 = [20, 45, 90, 180, 300]
    waits_other = [5, 15, 30, 60, 120]
    attempt_481 = 0
    attempt_other = 0
    ...
    while True:
        try:
            ...
            if attempt_other >= len(waits_other):
                raise RuntimeError(f"Errore rete persistente: {exc!r}")
            wait = waits_other[attempt_other]
            attempt_other += 1
            print(f"[RETRY RETE] attesa {wait}s")
            sleep_jitter(wait, 3)
            continue
        ...
        if r.status_code == 200:
            try:
                ...
                raise RuntimeError(f"JSON non valido da {r.url}: {exc!r}")

        if r.status_code == 481:
            if attempt_481 >= len(waits_481):
                raise RuntimeError(f"HTTP 481 persistente dopo retry: {r.url}")
            wait = waits_481[attempt_481]
            attempt_481 += 1
            print(f"[HTTP 481] blocco temporaneo; attesa {wait}s prima del retry")
            sleep_jitter(wait, 5)
            continue

        if r.status_code in (429, 500, 502, 503, 504):
            if attempt_other >= len(waits_other):
                raise RuntimeError(f"HTTP {r.status_code} persistente: {r.url}")
            wait = waits_other[attempt_other]
            attempt_other += 1
            print(f"[HTTP {r.status_code}] attesa {wait}s")
            sleep_jitter(wait, 3)
            continue
...
def parse_product(product, category_code, category_name, site, store, source_url, stamp):
    ...
    price_value = number(price.get("value")) if isinstance(price, dict) else None
    ...
    ref_value = number(ref.get("value")) if isinstance(ref, dict) else None
    ...
    stock_status = str(stock.get("stockLevelStatus") or "") if isinstance(stock, dict) else ""

    return (
        code, name, brand, category_code, category_name,
        price_value,
        str(price.get("formattedValue") or "") if isinstance(price, dict) else "",
        ref_value, ref_formatted, ref_unit, stock_status,
        str(product.get("url") or product.get("productUrl") or ""),
...
def main():
    ...
    audit = {
        "engine": "FAMILA_ENGINE_ANTIBLOCCO",
    ...
    for cat_index, (category_code, category_name) in enumerate(VERIFIED_CATEGORIES):
        print(f"\n=== {category_code} - {category_name} ===")
        try:
            first, first_url = fetch_category(session, args.site, args.store, category_code, 0)
            pagination = first.get("pagination") or {}
            total_results = int(pagination.get("totalResults") or 0)
            total_pages = int(pagination.get("totalPages") or
                              (math.ceil(total_results / 20) if total_results else 1))
            ...
            for page in range(total_pages):
                if page == 0:
                    ...
                    # Ritmo volutamente conservativo per evitare il blocco 481.
                    sleep_jitter(3.5, 2.0)
                    data, source_url = fetch_category(
                        session, args.site, args.store, category_code, page
                    )

                pagination_now = data.get("pagination") or {}
                current_page = pagination_now.get("currentPage")
                ...
                for product in products:
                    row = parse_product(
                        product, category_code, category_name,
                        args.site, args.store, source_url, stamp
                    )
                    if row:
                        unique[row[0]] = row
                        positive_seen += 1

                print(f"pagina {page+1}/{total_pages}: {len(products)} prodotti")

                # Pausa più lunga ogni 8 pagine.
                if (page + 1) % 8 == 0 and page + 1 < total_pages:
                    print("[PAUSA PREVENTIVA] 18-24 secondi")
                    sleep_jitter(18, 6)
            ...
            audit["categories"][category_code] = {
                ...
                "received": received,
                "pages": total_pages,
                "rows_with_positive_price_seen": positive_seen,
            }

            # Raffreddamento tra reparti.
            if cat_index + 1 < len(VERIFIED_CATEGORIES):
                print("[PAUSA TRA CATEGORIE] 15-22 secondi")
                sleep_jitter(15, 7)
    ...
    if audit["errors"]:
        con.close()
        tmp.unlink(missing_ok=True)
        audit["verdict"] = "FAIL_CLOSED_ANTIBLOCK_RETRIES_EXHAUSTED"
    ...
    metadata = {
        ...
        "catalog_engine": "FAMILA_ENGINE_ANTIBLOCCO",
        "base_site_id": args.site,
    ...
    duplicates = con.execute(
        "SELECT COUNT(*) FROM (SELECT product_id, COUNT(*) c FROM products "
        "GROUP BY product_id HAVING c > 1)"
    ).fetchone()[0]
    con.close()

    if db_count <= 0 or positive_count != db_count or duplicates != 0:
        tmp.unlink(missing_ok=True)
        audit.update({
            "verdict": "FAIL_CLOSED_FINAL_DB_AUDIT",
            "db_count": db_count,
    ...
    audit.update({
        "verdict": "FAMILA_ENGINE_ANTIBLOCCO_VALIDATED",
